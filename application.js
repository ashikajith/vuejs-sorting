var data =   [{"latitude": "52.986375", "user_id": 12, "name": "Christina McArdle", "longitude": "-6.043701"},
...
new Vue({
  ...
  methods: {
    calculateDistance: function(lat2, lon2){
      ...
      var mathPi = 0.017453292519943295;    // Math.PI / 180
      var cos = Math.cos;
      var a = 0.5 - cos((lat2 - dublinLatitude) * mathPi)/2 +
              cos(dublinLatitude * mathPi) * cos(lat2 * mathPi) *
              (1 - cos((lon2 - dublinLongitude) * mathPi))/2;
      return Number(earthRadius * Math.asin(Math.sqrt(a))).toFixed(1);
    },
    greaterThanHundred: function(distance){
      if (distance < 100){
        return true;
      }
    },
